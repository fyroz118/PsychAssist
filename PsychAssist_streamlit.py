"""
PsychAssist Web v4.1 - Clinical Decision Support (Streamlit)

Fixes in v4.1
-------------
* Replaced the invalid `st.canvas(...)` (does not exist in Streamlit) with
  `streamlit_drawable_canvas.st_canvas`.
* Handwritten notes are saved as base64 PNG data URLs (SQLite-friendly).
* Fixed swapped column indices when listing saved counselling notes.
* Fixed undefined `conn` in the Assessment and ADHD pages.
* Fixed the ADHD INSERT to match the `assessments` table schema
  (`severity_grade`/`differential` -> `adhd_severity`/`differential`).
* Fixed the placeholder count in the Assessment INSERT (31 cols, was 30 `?`).
* Minor polish: use_container_width, pandas-based CSV export, clearer chat.
...
symptom_categories = {
    "Mood Symptoms": ["Low mood", "Anhedonia", "Fatigue", "Hopelessness",
                      "Excessive guilt", "Suicidal thoughts", "Sleep disturbance", "Irritability"],
    "Mania Symptoms": ["Reduced sleep", "Increased energy", "Grandiosity",
                       "Pressured speech", "Racing thoughts", "Risk-taking behavior", "Distractibility"],
    "Psychotic Symptoms": ["Auditory hallucinations", "Visual hallucinations", "Delusions",
                           "Paranoia", "Thought broadcasting", "Disorganized speech", "Negative symptoms"],
    "Anxiety Symptoms": ["Panic attacks", "Excessive worry", "Palpitations",
                         "Sweating", "Tremor", "Avoidance behavior", "Fear of dying"],
    ...
    "Cognitive Symptoms": ["Memory loss", "Confusion", "Disorientation",
                           "Fluctuating attention", "Personality change", "Poor concentration"],
    "Neurological Symptoms": ["Seizure", "Weakness", "Tremor (neurological)",
                              "Gait disturbance", "Headache", "Loss of consciousness"],
...
medication_database = {
    ...
    "Bipolar I Disorder - Manic Episode": {
        "first_line": [
            {"name": "Lithium", "class": "Mood Stabilizer", "starting_dose": "300mg",
             "max_dose": "1800mg",
             "side_effects": "Tremor, polydipsia, polyuria",
             "contraindications": "Severe renal disease"},
            {"name": "Valproate", "class": "Anticonvulsant", "starting_dose": "250mg",
             "max_dose": "60mg/kg",
...
# Helpers
# ==========================================================================
def canvas_to_b64_png(image_data) -> str:
    """Convert the numpy RGBA array returned by st_canvas into a base64 PNG data URL."""
...
def depressive_logic(selected, duration, affect):
    ...
    for s in ["Fatigue", "Hopelessness", "Excessive guilt", "Suicidal thoughts",
        ...
        if has_s(selected, s):
            score += 2
    if sum(1 for s in ["Grandiosity", "Increased energy", "Reduced sleep"] if has_s(selected, s)) >= 2:
        score -= 8
...
def diagnose_mdd(selected, duration, impairment):
    cnt = sum(1 for s in ["Low mood", "Anhedonia", "Fatigue", "Hopelessness",
                          "Excessive guilt", "Suicidal thoughts", "Sleep disturbance",
                          "Poor concentration"] if has_s(selected, s))
...
def diagnose_mania(selected, duration):
    cnt = sum(1 for s in ["Reduced sleep", "Increased energy", "Grandiosity",
                          ...
                          "Risk-taking behavior", "Distractibility"] if has_s(selected, s))
    if cnt >= 4 and has_s(selected, "Reduced sleep") and has_s(selected, "Increased energy") \
...
def diagnose_schizophrenia(selected, duration):
    ...
    cnt = sum(1 for s in ["Auditory hallucinations", "Visual hallucinations", "Delusions",
...
def diagnose_delirium(selected, duration, fluctuating):
    if has_s(selected, "Confusion") and has_s(selected, "Disorientation") \
...
if page == "Assessment":
    ...
    with st.form("assessment_form"):
        ...
        for cat, items in symptom_categories.items():
            st.write(f"**{cat}**")
            for item in items:
                if st.checkbox(item, key=f"sym_{item}"):
        ...
        if submitted:
            if not name:
                ...
                dur_num = 1 if duration in ["Days","Weeks"] else 2 if duration in ["Months"] else 3
                ...
                filtered = {k:v for k,v in {"Depressive":dep_score, "Manic":man_score, "Psychotic":psych_score, "Delirium":del_score}.items() if v > 0}
                top_syndrome = max(filtered, key=filtered.get) if filtered else "Unknown"
                severity = severity_grader(max(filtered.values()) if filtered else 0)
                formal = []
                if diagnose_mdd(selected, duration, " ".join(func_imp)): formal.append(diagnose_mdd(selected, duration, " ".join(func_imp)))
                if diagnose_mania(selected, duration): formal.append(diagnose_mania(selected, duration))
                if diagnose_schizophrenia(selected, duration): formal.append(diagnose_schizophrenia(selected, duration))
                if diagnose_delirium(selected, duration, fluctuating): formal.append(diagnose_delirium(selected, duration, fluctuating))
                organic_level, organic_score = organic_psychosis_detector(selected, onset_s, fluctuating, seizure, focal, head_injury)
                risk_level, risk_rec, risk_score = risk_assessment(selected, risk_suicide, risk_command, risk_violent, risk_means)
                report_text = f"""
╔══════════════════════════════════════════════════════════════════════════════════╗
...
Syndrome: {top_syndrome} | Severity: {severity}

MSE: {generate_mse(speech, affect, thought, insight, judgment)}

Risk: {risk_level} ({risk_score}/20)
...
"""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (name, str(age), sex, ", ".join(selected), top_syndrome, severity, risk_level, str(formal),
                organic_level, organic_score, ", ".join(func_imp), generate_mse(speech, affect, thought, insight, judgment),
                duration, onset, pattern, speech, affect, thought, insight, judgment, ", ".join(substance),
                ", ".join(neuro), report_text, generate_ai_insights(top_syndrome, severity, risk_level, formal),
                str(diagnose_mdd(selected, duration, " ".join(func_imp))) if diagnose_mdd(selected, duration, " ".join(func_imp)) else "",
                str(diagnose_mania(selected, duration)) if diagnose_mania(selected, duration) else "",
                str(diagnose_schizophrenia(selected, duration)) if diagnose_schizophrenia(selected, duration) else "",
                str(diagnose_delirium(selected, duration, fluctuating)) if diagnose_delirium(selected, duration, fluctuating) else "",
                1 if mixed_features_detector(selected) else 0, calculate_symptom_weight(selected), str(datetime.now())))
                conn.commit()
                st.session_state["current_report"] = report_text
                st.session_state["current_ai"] = generate_ai_insights(top_syndrome, severity, risk_level, formal)
...
if page == "PHQ-9":
    ...
    for i, q in enumerate(PHQ9_QUESTIONS):
        scores[i] = st.slider(q, 0, 3, 0)
    total = sum(scores.values())
    st.success(f"Total: {total} | {phq9_severity(total)}")
    if st.button("Save PHQ-9"):
        conn = get_conn()
        c = conn.cursor()
        c.execute("INSERT INTO phq9_scores (patient_name, total_score, severity, answers, timestamp) VALUES (?, ?, ?, ?, ?)",
                  (st.session_state.get("patient_name", "Unknown"), total, phq9_severity(total), str(scores), str(datetime.now())))
        conn.commit()
        conn.close()
        st.success("Saved!")
...
if page == "GAD-7":
    ...
    for i, q in enumerate(GAD7_QUESTIONS):
        scores[i] = st.slider(q, 0, 3, 0)
    total = sum(scores.values())
    st.success(f"Total: {total} | {gad7_severity(total)}")
    if st.button("Save GAD-7"):
        conn = get_conn()
        c = conn.cursor()
        c.execute("INSERT INTO gad7_scores (patient_name, total_score, severity, answers, timestamp) VALUES (?, ?, ?, ?, ?)",
                  (st.session_state.get("patient_name", "Unknown"), total, gad7_severity(total), str(scores), str(datetime.now())))
        conn.commit()
        conn.close()
        st.success("Saved!")
...
if page == "ADHD":
    ...
    with col1:
        adhd_type = st.radio("ADHD Type", ["Adult (ASRS)", "Adult (CAARS)", "Child (ADHD-RS)"], horizontal=True)
        age_group = st.selectbox("Age Group", ["Adult", "Adolescent", "Child (<13 years)"])
    with col2:
        if adhd_type == "Adult (ASRS)":
            asrs_scores = {}
            asrs_questions = ["How often do you have trouble wrapping up the final details of a project, once the challenging parts have been done?","How often do you have difficulty getting things in order when you have to do a task that requires organization?","How often do you have problems remembering appointments or obligations?","How often do you find that you have to read instructions over and over to understand them?","How often do you find yourself starting a new task before finishing the previous one?","How often do you find that you have trouble focusing your attention when you need to focus hard?"]
            for i, q in enumerate(asrs_questions):
                asrs_scores[f"asrs_{i}"] = st.slider(q, 0, 4, 0)
            st.session_state.adhd_asrs = sum(asrs_scores.values())
        elif adhd_type == "Adult (CAARS)":
            caars_scores = {}
            caars_questions = ["I have difficulty concentrating on tasks.","I am easily distracted.","I find it hard to sit still in meetings.","I talk too much.","I interrupt others.","I lose things.","I forget appointments.","I have trouble organizing my daily activities.","I feel restless.","I have a short attention span."]
            for i, q in enumerate(caars_questions):
                caars_scores[f"caars_{i}"] = st.slider(q, 0, 4, 0)
            st.session_state.adhd_caars = sum(caars_scores.values())
        else:
            adhd_rs_scores = {}
            adhd_rs_questions = ["Often fidgets with hands or feet or squirms in seat","Leaves seat in classroom or in other situations in which remaining seated is expected","Runs about or climbs excessively in situations in which it is inappropriate","Has difficulty playing or engaging in leisure activities quietly","Is often on the go or acts as if 'driven by a motor'","Talks excessively","Blurts out answers before questions have been completed","Has difficulty awaiting turn","Interrupts or intrudes on others","Has difficulty organizing tasks and activities"]
            for i, q in enumerate(adhd_rs_questions):
                adhd_rs_scores[f"adhd_rs_{i}"] = st.slider(q, 0, 3, 0)
            st.session_state.adhd_rs = sum(adhd_rs_scores.values())
    if st.button("🔍 Calculate ADHD Score & Diagnosis"):
        if adhd_type == "Adult (ASRS)":
            ...
            threshold = 24
        severity = "None" if score <= threshold // 2 else "Mild" if score <= threshold else "Moderate" if score <= threshold * 1.5 else "Severe"
        st.success(f"**ADHD Score: {score}/{threshold}** | Severity: {severity}")
        st.subheader("Differential Diagnosis")
        diff = st.selectbox("Compare with", ["No other diagnosis", "Major Depression", "Generalized Anxiety", "Bipolar Disorder", "Learning Disorder", "Trauma/PTSD", "Sleep Disorder"])
        st.info(f"ADHD vs {diff}: Overlap possible in most cases.")
        ...
        cursor = conn.cursor()
        cursor.execute("INSERT INTO assessments (patient_name, syndrome, severity_grade, adhd_score, adhd_type, differential, treatment_recommendations, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                       (st.session_state.get("patient_name", "Unknown"), "ADHD", severity, score, adhd_type, diff, " ".join(["Non-stimulant" if severity in ["Mild","Moderate"] else "Stimulant", "Behavioral therapy"]), str(datetime.now())))
        conn.commit()
