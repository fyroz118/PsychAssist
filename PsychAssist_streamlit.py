...
# Sidebar
# ---------------------------------------------------------------------------
st.sidebar.title("PsychAssist Web v4.2")
...
# ---------------------------------------------------------------------------
# Counselling & Notes (uses streamlit-drawable-canvas)
# ---------------------------------------------------------------------------
if page == "Counselling & Notes":
    ...
    with ctrl1:
        draw_mode = st.radio(
            "Mode",
            ["freedraw", "line", "rect", "circle", "transform", "eraser"],
            horizontal=True,
            key="draw_mode",
        )
    with ctrl2:
        stroke_width = st.slider("Pen width", 1, 15, 2, key="stroke_width")
    ...
    with col_a:
        if st.button("Save Handwritten Image"):
            if canvas_result.image_data is not None:
                st.session_state.handwritten_notes = canvas_to_b64_png(
                    canvas_result.image_data
                )
                st.success("Handwriting captured. Click 'Save All' below to persist.")
            else:
