...
# Sidebar
# ---------------------------------------------------------------------------
st.sidebar.title("PsychAssist Web v4.3")
...
if page == "Counselling & Notes":
    ...
    # Palette (label -> hex). "Custom" opens a colour picker.
    PALETTE = {
        "Black":  "#000000",
        "Red":    "#E53935",
        "Blue":   "#1E88E5",
    ...
    # Eraser: use drawable-canvas eraser mode.
    if tool == "Eraser":
    ...
    canvas_result = st_canvas(
        fill_color="rgba(255, 165, 0, 0.0)",   # no shape fill
    ...
    with b2:
        if st.button("Clear canvas", use_container_width=True):
            # Bump the suffix -> Streamlit mounts a fresh, empty canvas widget.
    ...
    with b4:
        if st.button("Exit full-screen" if full_screen
                     else "Enter full-screen",
                     use_container_width=True):
            st.session_state.draw_fullscreen = not full_screen
            st.rerun()
